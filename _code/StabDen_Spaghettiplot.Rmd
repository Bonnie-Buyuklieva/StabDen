...
```{r}
#filename: 1_StabDen_SpaghettiPlot_.Rmd
...
ggplot(flattend1, aes(x = year, stratum = rdl_psd,alluvium = rdl_psd, y = YRpercent)) +
  geom_flow(aes(fill = rdl_psd), colour='white', width = 0) +
...
ggplot(flattend1, aes(x = year, stratum = rdl_psd,alluvium = rdl_psd, y = YRpercent)) +
  ...
  ggrepel::geom_label_repel(
    aes(label = ifelse(as.numeric(year) == 1 & YRpercent >2, paste(as.character(round(YRpercent,1)),'%'), NA)),
    stat = "stratum", size = 4, direction = "y", nudge_x = -.5
  ) +
  ggrepel::geom_label_repel(
    aes(label = ifelse(as.numeric(year) == 5 & YRpercent >2, paste(as.character(round(YRpercent,1)),'%'), NA)),
    stat = "stratum", size = 4, direction = "y", nudge_x = .5
  )+
  theme(legend.position = "none")
```
...
```{r, fig.height = 10.2, fig.width = 6.3}
...
```{r LND, fig.height = 10.2, fig.width = 6.3}
